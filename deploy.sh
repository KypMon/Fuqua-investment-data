...
# ---- optional: quick way to inspect the node-build stage ----
if [[ "$1" == "debug-node" ]]; then
    echo "### Building and entering node-build stage..."
    docker build \
    --target node-build \
    -t node-build-image \
    -f fa.Dockerfile \
    ...
    docker run -it --entrypoint /bin/sh node-build-image
    exit 0
fi
