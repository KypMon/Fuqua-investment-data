...
The backend API routes are served by ```gunicorn```, which is a Pythn WSGI HTTP server for Linux systems.  It provides support for multiple worker processes to handle concurrent user requests.
...
The ```index.js``` file added this import: ```import '@fsb/fw-auth/dist/fw-auth.js'```.  This is web component code that enables identification of the user who wants to use the app.
...
Gitlab's continuous integration/continuous deployment (CI/CD) capability is used for deployment (see ```.gitlab-ci.yml```).
