...
class LifeCycleService(object):
    ...
    def _ensure_vector(self, values: Sequence[float], label: str) -> np.ndarray:
    ...
    def life_cycle_simulation(
        ...
        *,
        ...
        returns = self._ensure_vector(vec_ret, "Return")
        cashflows = self._ensure_vector(vec_cf, "Cash flow")
    ...
    def _create_histogram(self, final_wealth: np.ndarray, *, bins: str | int = "auto") -> dict:
    ...
    def _compute_below_cutoff(self, wmat: np.ndarray, wmin_cutoff: float) -> dict:
    ...
    def run_life_cycle_analysis(
        ...
        *,
        ...
        wmat = self.life_cycle_simulation(vec_ret, vec_cf, w0=w0, nsim=nsim)
        final_wealth = wmat[:, -1]
        histogram = self._create_histogram(final_wealth, bins=bins)
        below_cutoff = self._compute_below_cutoff(wmat, wmin_cutoff)
        below_counts_array = np.asarray(below_cutoff["counts"], dtype=int)
        summary = self._summarize_final_wealth(final_wealth, wmin_cutoff, below_counts_array)
