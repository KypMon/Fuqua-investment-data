...
from src.services.backtesting_aux import BacktestingAux
...
class BacktestService(object):
    def __init__(self):
        ...
        self.backtesting_aux = BacktestingAux()

    def backtesting(self, start_date, end_date, tickers, allocation1, allocation2, allocation3, rebalancing, benchmark, start_balance):
        ...
        if max_min_date > start_date:
            ...
            self.logger.info(data_range_message)
        ...
        for p in range(3):
            if ind_alloc[p] == 1:
                ...
                results = self.backtesting_aux.backtesting_aux(start_date, end_date, tickers2, alloc, rebalancing, data_short, ff5, start_balance)
        ...
        for p in range(3): # Typically for portfolios 1-3 vs benchmark
            if ind_alloc[p] == 1 and ind_alloc[3] ==1: # Ensure benchmark (output[3]) exists
                ...
                if len(x_var) > 1: # Need at least 2 data points for regression
                    ...
                    self.logger.info(regression_warning)
