...
from src.services.matrix_payload import MatrixPayload
...
class MatrixService(object):
    ...
    def parse_matrix_payload(self, payload: Optional[dict]) -> pd.DataFrame:
    ...
    def _clean_date_column(self, df: pd.DataFrame, column: str = "Date") -> pd.DataFrame:
    ...
    def download_matret(
        ...
        matret = self._clean_date_column(matret, "Date")

        return matret, available


    def create_mat_er_covr(self, matret: pd.DataFrame, risk_free: Optional[float] = None) -> Tuple[pd.DataFrame, Optional[float]]:
    ...
    def _build_covariance(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[str], Optional[float]]:
        df = df.copy()
        df.columns = [str(col).strip() for col in df.columns]
    ...
    def _solve_min_variance(self, cov: np.ndarray, allow_short: bool) -> np.ndarray:
        n = cov.shape[0]
        P = matrix(cov)
        q = matrix(np.zeros(n))
        A = matrix(1.0, (1, n))
    ...
    def compute_portfolios(
        ...
        means, cov, assets, rf_from_matrix = self._build_covariance(mat_er_covr)
        ...
        def build_block(allow_short: bool) -> Dict[str, object]:
            min_var_weights = self._solve_min_variance(cov, allow_short)
            ...
            for target in ret_targets:
                weights = self._solve_target_return(cov, means, float(target), allow_short)
                if weights is None:
                    continue
