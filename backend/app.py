...
def create_app(config_file=None, log_file=None):
    ...
    """
    # Allow env vars as defaults (picked up by Gunicorn)
    ...
    # Example: pass log file into Flask config
...
# When imported by gunicorn (server environment), this line executes
#app = create_app()
...
#     # @SDS end
if __name__ == "__main__":
    # This block only runs when executed directly in localhost environment (python app.py ...)
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", dest="config_file", default="config/.env")
    parser.add_argument("--log", dest="log_file", default="app.log")
    args = parser.parse_args()

    # Build app with command-line arguments for local/dev use
